...
**AI:** Google Gemini REST API (2.5 Flash with automatic model cascading)  
**Auth:** JWT (access + refresh tokens), bcrypt, OTP via email  
**Maps:** OpenStreetMap Overpass API, Nominatim Geocoding  
**SMS:** Twilio  
...
```bash
git clone https://github.com/Ayush13harsh/AyuSphere.git
...
| `BREVO_API_KEY` | No | Brevo (Sendinblue) transactional email API key |
| `SMTP_FROM_EMAIL` | No | Sender email address for OTP emails |
| `TWILIO_ACCOUNT_SID` | No | Twilio account SID |
| `TWILIO_AUTH_TOKEN` | No | Twilio auth token |
| `TWILIO_PHONE_NUMBER` | No | Twilio sender number |
| `GOOGLE_MAPS_API_KEY` | No | Google Maps key (falls back to OpenStreetMap) |
