...
export default function AppLayout({ children, title = 'AyuSphere' }) {
    ...
    return (
        <>
            <header>
                <h1>
                    <img src="/logo.svg" alt="AyuSphere" width="30" height="30" style={{ borderRadius: '8px', filter: 'drop-shadow(0 4px 8px rgba(186,0,53,0.15))' }} />
                    <span style={{ fontFamily: 'Manrope', fontWeight: 800, color: 'var(--primary-guardian)' }}>{title}</span>
                </h1>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <button onClick={logout} className="btn-outline" style={{ width: 'auto', padding: '0.45rem 1.2rem', fontSize: '0.8rem', borderRadius: '12px', border: '1px solid var(--outline-variant)', fontWeight: 700, whiteSpace: 'nowrap' }}>
                        Sign Out
                    </button>
                </div>
            </header>
